...
// Ensure the mapping inside the JSX uses basketItems directly since enhancedItems is gone!
code = code.replace(/enhancedItems\.map/g, 'basketItems.map');

// Replace the entire footer manually by replacing everything from "Required Margin" down to the second to last div.
// It's safer to just replace the whole return block if we must, but let's just use string slice for safety.
const footerStartStr = `<div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '24px', paddingTop: '16px', borderTop: '1px solid rgba(255,255,255,0.05)' }}>`;
const footerEndStr = `</button>\n          </div>`; // Based on typical JSX closing tags
const idxF = code.lastIndexOf(footerStartStr);
if(idxF > -1) {
   // find the closing of the main modal body
   code = code.slice(0, idxF) + `<LiveBasketFooter basketItems={basketItems} productType={productType} placeBasketOrder={placeBasketOrder} setBasketModalOpen={setBasketModalOpen} balanceNum={balanceNum} />\n        </div>\n      </div>\n    </div>\n  );\n}\n`;
}

fs.writeFileSync('frontend/src/components/BasketModal.jsx', code);
console.log('Fixed BasketModal correctly');
