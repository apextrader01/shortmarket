...
function enrichLotsize(token, uniqueSymbol) {
    if (token && STOCK_MASTER[token] && !STOCK_MASTER[token].lotsize) {
        ...
        if (found) {
            STOCK_MASTER[token].lotsize = Number(found.lotsize || 1);
        } else {
            STOCK_MASTER[token].lotsize = 1;
        }
    }
}
        ...
        global_web_socket.on('tick', (receiveData) => {
            ...
            for (const data of dataArray) {
                ...
                if (Object.keys(data).length > 8) {
                    console.log(`[DEBUG SNAPQUOTE TICK ${info?.uniqueSymbol}]:`, Object.keys(data));
                }
                
                if (info && data.best_5_buy_data && data.best_5_sell_data) {
                    console.log(`[DEPTH] Received depth data for ${info.uniqueSymbol}`);
                    ...
                    const depthData = {
                        ...
                        open: data.open_price_day ? (data.open_price_day / 100).toFixed(2) : null,
                        high: data.high_price_day ? (data.high_price_day / 100).toFixed(2) : null,
                        low: data.low_price_day ? (data.low_price_day / 100).toFixed(2) : null,
                        close: data.close_price ? (data.close_price / 100).toFixed(2) : null,
                        ...
                        avgPrice: data.avg_traded_price ? (data.avg_traded_price / 100).toFixed(2) : null,
                        ltt: data.last_traded_timestamp ? new Date(data.last_traded_timestamp * 1000).toLocaleString('en-GB') : null,
                        lowerCircuit: data.lower_circuit ? (data.lower_circuit / 100).toFixed(2) : null,
                        upperCircuit: data.upper_circuit ? (data.upper_circuit / 100).toFixed(2) : null,
                    };
