...
// 1. Add fetchAdminWithdrawals and processAdminWithdrawal to store destructing
const targetStore = `adminMasterSquareOff: state.adminMasterSquareOff, fetchAdminUsers: state.fetchAdminUsers,`;
const replaceStore = `adminMasterSquareOff: state.adminMasterSquareOff, fetchAdminWithdrawals: state.fetchAdminWithdrawals, processAdminWithdrawal: state.processAdminWithdrawal, fetchAdminUsers: state.fetchAdminUsers,`;
content = content.replace(targetStore, replaceStore);
content = content.replace(`fetchAdminLedger, forceCloseUserPosition, 
adminResetUser, adminDeleteUser, updateUserDetails , toggleUserBan, adminMasterSquareOff } =`, `fetchAdminLedger, forceCloseUserPosition, 
adminResetUser, adminDeleteUser, updateUserDetails , toggleUserBan, adminMasterSquareOff, fetchAdminWithdrawals, processAdminWithdrawal } =`);

// 2. Add state for withdrawals
const targetState = `  const [deposits, setDeposits] = useState([]);`;
const replaceState = `  const [deposits, setDeposits] = useState([]);
  const [withdrawals, setWithdrawals] = useState([]);`;
content = content.replace(targetState, replaceState);

// 3. Update loadData to fetch withdrawals
const targetLoad = `        fetchAdminLedger(page, 50, debouncedSearch).then(res => { setLedger(res.data); setTotalPages(res.pagination.totalPages); })
      ]);
    } else if (activeTab === 'deposits') {`;
const replaceLoad = `        fetchAdminLedger(page, 50, debouncedSearch).then(res => { setLedger(res.data); setTotalPages(res.pagination.totalPages); })
      ]);
...
// 4. Add Tab Button
const targetTabBtn = `              onClick={() => setActiveTab('deposits')} 
...
const replaceTabBtn = `              onClick={() => setActiveTab('deposits')} 
              style={{ background: 'none', border: 'none', padding: '8px 0', borderBottom: activeTab === 'deposits' ? '2px solid var(--color-blue)' : '2px solid transparent', color: activeTab === 'deposits' ? 'var(--text-primary)' : 'var(--text-secondary)', fontWeight: activeTab === 'deposits' ? '600' : '500', cursor: 'pointer', whiteSpace: 'nowrap' }}
            >
              Deposit Requests
            </button>
...
content = content.replace(targetTabBtn, replaceTabBtn);

// 5. Add Withdrawals Table UI
const targetTable = `        ) : activeTab === 'deposits' ? (`;
