...
const target = `updateUserDetails , toggleUserBan } = useStore(useShallow(state => ({ toggleUserBan: 
state.toggleUserBan, fetchAdminUsers: state.fetchAdminUsers,`;
const replacement = `updateUserDetails , toggleUserBan, adminMasterSquareOff } = useStore(useShallow(state => ({ toggleUserBan: 
state.toggleUserBan, adminMasterSquareOff: state.adminMasterSquareOff, fetchAdminUsers: state.fetchAdminUsers,`;

content = content.replace(target, replacement);

const targetHeader = `<table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>`;
const replacementHeader = `<div style={{ padding: '16px', display: 'flex', justifyContent: 'flex-end', borderBottom: '1px solid var(--border-color)' }}>
            <button
              onClick={async () => {
