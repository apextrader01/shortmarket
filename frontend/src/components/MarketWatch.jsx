    ...
    >
      ...
      {(isHovered || isSearchMode) && (
        <div style={{ display: 'flex', alignItems: 'center', gap: '6px', paddingRight: '8px' }}>
          {isSearchMode ? (
            isInWatchlist ? (
              <div onClick={(e) => { e.stopPropagation(); removeStockFromWatchlist(activeWatchlistId, stock.uniqueSymbol); }} style={{ padding: '3px', background: 'var(--color-red)', borderRadius: '4px', display: 'flex' }}><Minus size={12} color="#fff" /></div>
            ) : (
              <div onClick={(e) => { e.stopPropagation(); addStockToWatchlist(activeWatchlistId, stock.uniqueSymbol); }} style={{ padding: '3px', background: 'var(--color-blue)', borderRadius: '4px', display: 'flex' }}><Plus size={12} color="#fff" /></div>
            )
          ) : (
            <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
              ...
              <div onClick={(e) => { e.stopPropagation(); removeStockFromWatchlist(activeWatchlistId, stock.uniqueSymbol); }} style={{ padding: '3px', background: 'var(--border-color)', borderRadius: '3px', display: 'flex', marginLeft: '2px' }} title="Remove"><Trash2 size={12} color="var(--color-red-light)" /></div>
              <div onClick={(e) => { e.stopPropagation(); openMarketDepthModal(stock.uniqueSymbol, currentLotsize); }} style={{ padding: '3px', background: 'var(--border-color)', borderRadius: '3px', display: 'flex', marginLeft: '2px' }} title="Market Depth"><AlignRight size={12} color="var(--color-blue)" /></div>
            </div>
          )}
        </div>
      )}
      ...
      {data && !(isHovered || isSearchMode) ? (
        <div style={{ textAlign: 'right', flexShrink: 0 }}>
          <div key={data.last_update_time || data.ltp} className={data.tickDirection === 1 ? 'flash-up' : data.tickDirection === -1 ? 'flash-down' : ''} style={{ fontWeight: '600', fontSize: '12px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: '4px', padding: '2px 4px', color: isUp ? 'var(--color-green-light)' : isDown ? 'var(--color-red-light)' : 'var(--text-primary)' }}>
            {data.ltp.toFixed(2)}
