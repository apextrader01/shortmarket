...
const new_fetch = `async function fetchBatchLTPs(symbols) {
...
code = code.replace(/async function fetchBatchLTPs\(symbols\) \{[\s\S]*?(?=\n\/\/ (─── API FETCH FUNCTIONS|Fyers interval mapping))/m, new_fetch + "\n");
...
fs.writeFileSync('backend/services/fyers.js', code, 'utf8');
console.log("Done");
