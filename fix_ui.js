...
const storeRegex = /fetchAdminLedger\s*\}\s*=\s*useStore/g;
content = content.replace(storeRegex, 'fetchAdminLedger, fetchAdminTelemetry, adminTelemetry } = useStore');

const fetchRegex = /\} else if \(activeTab === 'withdrawals'\) \{/g;
content = content.replace(fetchRegex, `} else if (activeTab === 'telemetry') {
      fetchAdminTelemetry();
    } else if (activeTab === 'withdrawals') {`);

const tabRegex = /Withdrawals[\s\S]*?<\/button>[\s\S]*?<\/div>/g;
...
const uiRegex = /<\/table>\s*\)\s*:\s*null\}\s*<\/div>/g;
const uiReplacement = `</table>
        ) : activeTab === 'telemetry' ? (
